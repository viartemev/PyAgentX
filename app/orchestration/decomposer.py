...
- `edit_file_tool(path: str, content: str)`: Создает или полностью перезаписывает файл.
...
1.  **Полные пути**: ВСЕГДА используй полные относительные пути от корня проекта для любых файлов. Например: `app/agents/tools.py`, `tests/test_tools.py`.
2.  **Конкретика**: Формулируй задачи максимально конкретно. Вместо "написать функцию", пиши "добавить функцию X в файл Y".
3.  **Логика прежде тестов**: План должен сначала содержать шаг для написания или изменения **полной логики** функции, и только потом — шаг для написания тестов.
4.  **Сфокусированное тестирование**: Шаг для тестирования должен указывать конкретный файл с тестами. Например: "Запустить тесты в `tests/test_tools.py`".
5.  **Назначение**: Для каждого шага укажи `assignee` (`CodingAgent`, `TestingAgent`, `DefaultAgent`).
6.  **Формат**: Вывод должен быть СТРОГО в формате JSON-массива объектов.
...
[
    {
        ...
        "task": "Добавить функцию 'multiply(a, b)' в файл 'app/agents/tools.py'.",
        "description": "Нужно открыть файл 'app/agents/tools.py' и добавить в него новую Python-функцию 'multiply', которая принимает два числовых аргумента и возвращает их произведение. Использовать режим 'append' для инструмента edit_file."
    },
    {
        "step": 2,
        "assignee": "CodingAgent",
        "task": "Создать или обновить файл 'tests/test_tools.py' с тестами для 'multiply'.",
    ...
    },
    {
        ...
        "description": "Используя инструмент 'run_tests', запустить тесты, указав путь 'tests/test_tools.py', и проверить, что все тесты выполняются успешно."
    }
]
```
"""
